...
public class WPTree implements WordPath {
	...
	*/
	public List<String> findPath(String word1, String word2) 
	{
		...
		HashSet<String> visited=new HashSet<String>();// store every words
		visited.add(root.getWord());
		WPTreeNode tempNode=root;
		List<String> Children=new ArrayList<String>();
		Queue <WPTreeNode> VNodes=new LinkedList<WPTreeNode>();
		VNodes.add(root);
		//boolean finded=false;
		while(!VNodes.isEmpty()) {
			tempNode=VNodes.remove();
			Children=nw.distanceOne(tempNode.getWord(), true);
			for(String str: Children ) {
				if(!visited.contains(str)) {
					tempNode.addChild(str);
					visited.add(str);
				}
				if(str.contains(word2)) {
					//finded=true;
					WPTreeNode FinalNode=new WPTreeNode(word2,tempNode);
					Path=FinalNode.buildPathToRoot();
					return Path;
				}
				
			}
			for(WPTreeNode node:tempNode.getChildren()) {
				VNodes.add(node);
			}
	...
	public static void main(String[] args) {
		WPTree mmp=new WPTree();
		System.out.println(mmp.findPath("smart", "fool"));
	}
